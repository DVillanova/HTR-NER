...
########################################################################################
##The lines are extracted - commented because only execute once
...
##################################################################################################################################################################################
#Preparation files without suffixes
# rm ${PartDir}/"test".lst
...
#Creation of the index.words (files with transcription + labels)
#Eliminate URL part: '...' and .jpg at the end    
...
#just in case remove .JPG
sed 's/.JPG"//g' -i index.words-german
sed 's/.JPG"//g' -i index.words-czech
sed 's/.JPG"//g' -i index.words-latin

#Pile all index.words in one and preprocess
...
for f in index.words; do
	echo "Procesado de " ${f}
  sed 's/Č/C/g' -i $f; #Change by c
  echo "Č"
done


##########################################################
# Parser NEs with parentheses -> NEs continuous notation #
##########################################################
...
if $(which compute-wer &> /dev/null); then
  ...
  python $ScriptsDir/fix_separate_symb_output.py $TmpDir/decode/lattices/SeparateSimbols-wordtest.txt $TmpDir/decode/lattices/fixed_SeparateSymbols-wordtest.txt
  python $ScriptsDir/fix_separate_symb_output.py $TmpDir/decode/lattices/SeparateSimbols-word.txt $TmpDir/decode/lattices/fixed_SeparateSymbols-word.txt
