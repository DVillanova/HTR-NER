...
# Estimate tag n-gram
# python $ScriptsDir/filter_named_entities.py ./char.total.txt ./tag.total.txt
#ngram-count -vocab ../tags.lst -text ../char/tag.total.txt -lm lang/tag_LM.arpa -order 2 -wbdiscount1 -kndiscount -interpolate
...
$ScriptsDir/extractNERHip.sh $TmpDir/decode/lattices/word-lm/hyp_word-test.txt .
...
$ScriptsDir/extractNERHip2.sh $TmpDir/decode/lattices/word-lm/combined_hyp_word-test.txt .
for f in *; do sed 's/^ //g' -i $f; done
...
$ScriptsDir/extractNERHip2.sh $TmpDir/decode/lattices/word-lm/aligned_hyp_word-test.txt .
for f in *; do sed 's/^ //g' -i $f; done
...
$ScriptsDir/extractNER-GT.sh $TmpDir/test.txt .
for f in *; do sed 's/^ //g' -i $f; done
...
python $ScriptsDir/dist_edicion_custom_saturated.py ./NER-GT ./COMBINED_NER-DECODE >> $WorkDir/res_exp.txt

python $ScriptsDir/calc_prec_rec.py ./NER-GT ./ALIGNED_NER-DECODE >> $WorkDir/res_exp.txt
python $ScriptsDir/dist_edicion_custom_saturated.py ./NER-GT ./ALIGNED_NER-DECODE >> $WorkDir/res_exp.txt
...
mkdir NER
cd NER
$ScriptsDir/extractNERHip.sh ../lattices/word-lm/hyp_word-test.txt .
for f in *; do sed 's/^ //g' -i $f; done 
cd ..

mkdir NER2
cd NER2
$ScriptsDir/extractNERHip2.sh ../lattices/word-lm/hyp_word-test.txt .
for f in *; do sed 's/^ //g' -i $f; done
cd ..

mkdir NER-NoRep
cd NER-NoRep
$ScriptsDir/extractNERHip-noRep.sh ../lattices/word-lm/hyp_word-test.txt .
for f in *; do sed 's/^ //g' -i $f; done

$ScriptsDir/evaluacioNER.sh ../NER/ ./NER/ ../PARTITIONS/test-lines.lst 
$ScriptsDir/evaluacioNER.sh ../NER/ ./NER2/ ../PARTITIONS/test-lines.lst
$ScriptsDir/evaluacioNER.sh ../NER-NoRep/ ./NER-NoRep/ ../PARTITIONS/test-lines.lst

##Ficheros para Chris Kermovant

for f in $(<../PARTITIONS/test-lines.lst); do echo "line "`basename ${f/.png/}`; echo "line "`basename ${f/.png/}` > list; $ScriptsDir/evaluacioNER.sh ../NER/ ./NER2/ list ; done 2> /tmp/kk > kk
