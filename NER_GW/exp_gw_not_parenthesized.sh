...
#CREATION OF .TXT FILES WITH LINE ID AND TRANSCRIPTION
#FROM .LST FILES (PARTITION FILES) AND INDEX.WORDS
rm -rf ${TmpDir}
mkdir ${TmpDir}
cd ${TmpDir}
...
simplex.py -v -m "/root/directorioTrabajo/DOC-NER/HTR-NER/scripts/opt_gsf-wip_cl.sh {$ASF} {$WIP}" > result-simplex
...
# Use the category language model and generate output
...
#Rescoring lattice -> Language Model -> Generate output
...
# Recombine the outputs to take the Output of the Rescore and the other in case of doubt
...
#NER NORMAL
python $ScriptsDir/calc_prec_rec.py ./NOT_PARENTHESIZED_NER-GT ./NOT_PARENTHESIZED_NER-DECODE >> $WorkDir/res_exp.txt
python $ScriptsDir/alt_dist_edicion_custom_saturated.py ./NOT_PARENTHESIZED_NER-GT ./NOT_PARENTHESIZED_NER-DECODE >> $WorkDir/res_exp.txt
