...
python $ScriptsDir/generate_tagged_line_transcription.py \
...
#Extract vocab and number of symbols in vocabulary
cat char.total.txt | cut -f 2- -d\  | tr \  \\n| sort -u -V | awk 'BEGIN{   N=0;   printf("%-12s %d\n", "<eps>", N++);   printf("%-12s %d\n", "<ctc>", N++);  }NF==1{  printf("%-12s %d\n", $1, N++);}' >  symb.txt
# NSYMBOLS=$(sed -n '${ s|.* ||; p; }' "symb.txt");

cd ${WorkDir}

#CREATE FILES WITH LINE ID - TRANSCRIPTION
#FROM INDEX.WORDS AND .LST FILES (PARTITION FILES)
#SAVE ON WorkDir/TMP/
rm -rf ${TmpDir}
mkdir ${TmpDir}
cd ${TmpDir}
...
# Get word-level transcript hypotheses (CALCULATE WER)
awk '{
  printf("%s ", $1);
  for (i=2;i<=NF;++i) {
    if ($i == "<space>")
...
#Align output to generate hypothesis with transc 1-best and correct tagging
...
if $(which compute-wer &> /dev/null); then
  ...
  python $ScriptsDir/fix_separate_symb_output_alt.py $TmpDir/decode/SeparateSimbols-wordtest.txt $TmpDir/decode/fixed_SeparateSimbols-wordtest.txt
  python $ScriptsDir/fix_separate_symb_output_alt.py $TmpDir/decode/SeparateSimbols-word.txt $TmpDir/decode/fixed_SeparateSimbols-word.txt
  compute-wer --mode=present ark:fixed_SeparateSimbols-word.txt ark:fixed_SeparateSimbols-wordtest.txt >> $WorkDir/res_exp.txt
...
rm -rf $WorkDir/PREC-REC/ALT_NOT_PARENTHESIZED_NER-GT
mkdir $WorkDir/PREC-REC/ALT_NOT_PARENTHESIZED_NER-GT
cd $WorkDir/PREC-REC/ALT_NOT_PARENTHESIZED_NER-GT
...
rm -rf $WorkDir/PREC-REC/ALT_NOT_PARENTHESIZED_NER-DECODE
mkdir $WorkDir/PREC-REC/ALT_NOT_PARENTHESIZED_NER-DECODE
cd $WorkDir/PREC-REC/ALT_NOT_PARENTHESIZED_NER-DECODE
python $ScriptsDir/extract_ner_not_parenthesized_gw.py $TmpDir/decode/fixed_SeparateSimbols-wordtest.txt ./
for f in *; do sed 's/^ //g' -i $f; done
cd ..
...
#NER NORMAL
python $ScriptsDir/calc_prec_rec.py ./ALT_NOT_PARENTHESIZED_NER-GT ./ALT_NOT_PARENTHESIZED_NER-DECODE >> $WorkDir/res_exp.txt
python $ScriptsDir/alt_dist_edicion_custom_saturated.py ./ALT_NOT_PARENTHESIZED_NER-GT ./ALT_NOT_PARENTHESIZED_NER-DECODE >> $WorkDir/res_exp.txt


# echo "ORACLE NER RESULTS" >> $WorkDir/res_exp.txt
# python $ScriptsDir/calc_prec_rec.py ./NER-GT ./ORACLE_NER >> $WorkDir/res_exp.txt
# python $ScriptsDir/alt_dist_edicion_custom_saturated.py ./NER-GT ./ORACLE_NER >> $WorkDir/res_exp.txt