...
#FROM .LST FILES (PARTITIONS) AND INDEX.WORDS
rm -rf ${TmpDir}
mkdir ${TmpDir}
cd ${TmpDir}
...
#Align the output to generate hypothesis with transc 1-best and correct tagging
...
sed 's/\([.,:;?]\)/ \1/g;s/\([¿¡]\)/\1 /g' word-lm/combined_hyp_word-test.txt > SeparateSimbols-combined_wordtest.txt
sed 's/ \.line/\.line/g;s/ \.r\([0-9]\)/\.r\1/g' -i SeparateSimbols-combined_wordtest.txt
sed 's/</ </g' SeparateSimbols-combined_wordtest.txt | sed 's/>/> /g' | sed 's/  / /g' > k; mv k SeparateSimbols-combined_wordtest.txt
